  ...
  for (int i=0;i<sequence_list.size();i++){
    ...
    for (int j=0;j<sequence_list[i].size()-4;j++){
      ...
      if (fragment.find_first_not_of("ATGC")!=std::string::npos){
		...
		// Add by Tsu-Pei
		if(fragment.compare("AAATT")==0){
			heptamer = sequence_list[i].substr(j-1,7);
			std::cout<<"heptamer:"<<heptamer<<std::endl;
			if (found_str_in_map(heptamer,onemap)){
				pointer_v = &onemap[heptamer]; 
			}
		}

		std::cout<<fragment<<"oooo"<<std::endl;
	
      }
      else{
		pointer_v = &onemap[opposite_strand(fragment)];
		int_v=-1;
		std::cout<<fragment<<"XXXX"<<std::endl;
